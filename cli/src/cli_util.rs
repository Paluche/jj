...
pub async fn visit_collapsed_untracked_files(
    ...
    let non_collapsable: Vec<String> = other_non_tracked_paths
        .into_iter()
        .filter_map(|p| {
            p.as_ref()
                .parent()
    ...
    'untracked: for (path, is_dir) in untracked_paths {
        ...
        if let Some((i, component)) = first_mismatch {
            ...
            for component in std::iter::once(component).chain(it) {
                ...
                // Path should not contain other non tracked paths.
                } else if non_collapsable
                    .iter()
                    .any(|p| p == path.as_ref().as_internal_file_string())
                {
                    // non_collapsable contains only directories.
                    continue;
                } else {
...
mod tests {
    ...
    fn test_collapsed_files() {
        ...
        insta::assert_snapshot!(
        collect_collapsed_tracked_files_string(&tracked_files, &untracked_files, &ignored_paths, &tree, "A "),
        @r"
        A top_level_file
