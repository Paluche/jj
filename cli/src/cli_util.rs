...
pub async fn visit_collapsed_tracked_files(
    ...
    async fn is_collapsable(
        ...
        {
            // No dangling ignored files within.
            false
        } else if ignored_dirs.iter().any(|p| parent.starts_with(p))
        {
    ...
    let ignored_dirs = ignored_paths.iter().filter_map(|(p, is_dir)| is_dir.then_some(p)).collect();

    let ignored_files = ignored_paths.iter().filter_map(
        |(p, is_dir)| (!is_dir).then_some(p)).collect();
    ...
    while let Some((parent, paths)) = candidates.pop_first() {
        ...
        .await?
        {
            ...
            if num_file == 1 {
                candidates.insert(parent.parent().unwrap(), paths);
            } else {
                candidates.insert(parent.parent().unwrap(), vec![(parent, num_file, true)]);
            }
        } else {
...
mod tests {
    ...
    fn test_collapsed_files() {
        ...
        let ignored = files
            .into_iter()
            .filter_map(|(repo_path, file_state)| {
                matches!(file_state, FileState::Ignored).then_some((*repo_path, false))
            })
            .collect_vec();

        // TODO: The collapse is incorrect as it does not take the ignored
        // path into account.
        insta::assert_snapshot!(
            collect_collapsed_untracked_files_string(&untracked, &tree),
            @r"
        ? untracked_top_level_file
        ? partially_tracked/e
        ? untracked/
        ? fully_untracked/
        ? dir/partially_tracked/r
        ? dir/untracked/
        ? dir/fully_untracked/
        "
        );
        ...
        insta::assert_snapshot!(
        collect_collapsed_tracked_files_string(&tracked, &untracked, &ignored, &tree, "A "),
        @r"
        A top_level_file
        A dir/fully_tracked/ (2 files)
        A dir/partially_tracked/q
        A dir/tracked/m
